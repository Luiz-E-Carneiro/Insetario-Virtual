...
export default function Header () {
    ...
    return (
        <header className="fixed top-0 left-0 w-full h-14 shadow-md z-30 bg-white">
            <div className="flex items-center justify-between h-full px-10 max-sm:p-4">

                <div className="flex items-center gap-2 font-bold text-lg sm:text-xl md:text-2xl cursor-pointer">
                    <img src={logo_insetario} alt="Logo do Insetário Virtual" className="h-11 sm:h-12 rounded-full" />
                    <span>Insetário Virtual</span>
                </div>

                <nav
                    className={`absolute sm:static top-14 sm:top-0 right-0 w-fit sm:w-auto sm:flex items-center bg-white sm:bg-transparent transition-all duration-500 ease-in-out overflow-hidden ${open ? 'max-h-[300px] opacity-100' : 'max-h-0 opacity-0'} sm:max-h-none sm:opacity-100`}>
                    <ul className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center w-fit sm:w-auto">
